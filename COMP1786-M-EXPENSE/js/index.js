...
function registerTrip(e) {
    ...
    db.transaction(function (tx) {
        let query = `INSERT INTO Trip (Name, Destination, Description,Note, Risk, Date) VALUES (?, ?, ?, ?,?, julianday('${date}'))`;
        tx.executeSql(query, [name, destination, description,note, risk], transactionSuccess, transactionError);
...
function updateTrip(e) {
    ...
    db.transaction(function (tx) {
        let query = `UPDATE Trip SET Name = ?, Destination = ?, Description = ?, Note = ?, Risk = ?, Date = julianday('${date}') WHERE Id = ?`;

        tx.executeSql(query, [name, destination, description,note, risk, id], transactionSuccess, transactionError);
